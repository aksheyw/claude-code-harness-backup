...
**It fails loudly rather than silently.** Every copy is checked. If any copy fails, the failure is named in the report, the closing message says the capture is incomplete, and the script exits `1`. Exit `0` means everything it tried to copy, it copied. Exit `2` means it refused to start. People read this report and then erase a disk, so a copy that quietly failed while the report said "Copied" would cost someone their work.
